...
(async () => {
  ...
  fs.writeFileSync(
    path.join(dir, "api-search-page1.json"),
    JSON.stringify(search.body, null, 2),
  );
  console.log(
    `search fixture : ${search.body.data.length} items · ` +
      `total=${search.body.meta.total} · last_page=${search.body.meta.last_page}`,
  );

  const listing = await fetchJson(
    `https://olx.ba/api/listings/${LISTING_ID}`,
    20000,
  );
  fs.writeFileSync(
    path.join(dir, "api-listing-detail.json"),
    JSON.stringify(listing.body, null, 2),
  );
  const attrs = Array.isArray(listing.body.attributes)
    ? listing.body.attributes
    : [];
  console.log(
    `listing fixture: id=${listing.body.id} · views=${listing.body.views} · ` +
      `attributes=${attrs.length} · price_history=` +
      `${Array.isArray(listing.body.price_history) ? listing.body.price_history.length : "-"}`,
  );
  if (attrs.length)
    console.log("attribute sample:", JSON.stringify(attrs.slice(0, 3)));
