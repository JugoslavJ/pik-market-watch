...
(async () => {
  ...
  const { cards, meta } = parseSearchPage({ data: page.items, meta: page.meta });
  console.log(`search    : ${cards.length} cards · total=${meta.total} · ` +
              `last_page=${meta.lastPage} · rate left ${page.remaining}/${page.limit}`);
  if (!cards.length) throw new Error('no cards parsed from a non-empty payload');

  const priced = cards.find(c => c.price != null && c.sqm != null) || cards[0];
  console.log('card      :', JSON.stringify({
    articleId: priced.articleId, title: priced.title.slice(0, 48),
    price: priced.price, sqm: priced.sqm, rooms: priced.rooms,
    ppm2: priced.ppm2, isRent: priced.isRent,
    pin: [priced.latitude, priced.longitude], sellerType: priced.sellerType,
  }));

  const d = parseListingDetail(
    await fetchListing(priced.articleId, 20000), priced.articleId);
  console.log('detail    :', JSON.stringify({
    articleId: d.articleId, sqm: d.sqm, views: d.views,
    condition: d.condition, publishedAt: d.publishedAt,
