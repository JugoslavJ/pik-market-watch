...
const meta = (total, lastPage, current) =>
  ({ total, last_page: lastPage, current_page: current });
...
const baseCfg = overrides => ({
  maxPages: 30, concurrency: 2, pageDelayMs: 0, perPage: 40,
  apiTimeoutMs: 5, maxGeoFetches: 25, geoConcurrency: 2, geoDelayMs: 0,
  ...overrides,
});

/** Recording db double — enough surface for scrapeSearch, nothing more. */
function fakeDb(queueImpl) {
  const rec = { startRun: 0, savedSearches: [], upserts: [], refreshed: [],
                enriched: [], finishedRuns: [] };
  return {
    rec,
    async startRun() { rec.startRun += 1; return 1; },
    async registerSavedSearch(s) { rec.savedSearches.push(s); },
    async saveCards(cards) { rec.savedCards = cards; return { newCount: cards.length, dropCount: 0 }; },
    async upsertSavedSearch(u) { rec.upserts.push(u); },
    async refreshSearchResults(_key, ids) { rec.refreshed.push(ids); },
    async enrichmentQueue(ids, cap) {
      return queueImpl ? queueImpl(ids, cap) : { pending: [], total: ids.length };
    },
    async enrichListings(rows) { rec.enriched.push(...rows); },
    async finishRun(_id, info) { rec.finishedRuns.push(info); },
    async hasRecentFinishedRun() { return false; },
  };
}
...
test('single-page search: one fetch, ok run, correct stats and refresh', async () => {
  const db = fakeDb();
  const fetchPage = pageFetcher({ 1: { items: [rawCard(1), rawCard(2)], meta: meta(2, 1, 1) } });
  const res = await run(db, {}, { fetchSearchPage: fetchPage });

  assert.deepEqual(fetchPage.fetched, [1]);
  assert.deepEqual(res, { pages: 1, cards: 2, newCount: 2, dropCount: 0, enriched: 0 });
...
test('multi-page search: waves cover pages 2..last_page', async () => {
  ...
  assert.deepEqual([...fetchPage.fetched].sort((a, b) => a - b), [1, 2, 3]);
...
test('MAX_PAGES caps pagination even when last_page is huge', async () => {
  ...
  assert.deepEqual([...fetchPage.fetched].sort((a, b) => a - b), [1, 2]);
...
test('blank page 1: one retry, then loud failure with an error run record', async () => {
  ...
  const fetchPage = async url => {
    if (Number(url.searchParams.get('page')) !== 1) throw new Error('should stop at page 1');
    page1Visits += 1;
    return { items: [], meta: meta(0, 1, 1) };
  };
  const db = fakeDb();

  await assert.rejects(run(db, {}, { fetchSearchPage: fetchPage }), /0 listings after retry/);
...
test('enrichment: detail calls only where search cards cannot answer', async () => {
  ...
  const fetchDetailsInBatches = async ids => {
    detailIds.push(...ids);
    return ids.map(id => ({
      articleId: id, sqm: 99, latitude: 44.5, longitude: 17.1,
      views: 42, characteristics: { 'grijanje': 'plin' },
      apiStatus: 'active', apiPriceHistory: null,
      publishedAt: new Date('2026-01-01'), renewedAt: null,
    }));
  };

  const fetchPage = pageFetcher({
    1: { items: [rawCard(10), cardWithSqm, cardNoSqm, cardNoPin, cardWithPin],
         meta: meta(5, 1, 1) },
  });

  const res = await run(db, {}, { fetchSearchPage: fetchPage, fetchDetailsInBatches });
...
test('enrichment merge: null/empty detail fields never clobber known facts', async () => {
  const db = fakeDb(() => ({
    pending: [{ id: 30, unpinned: false, missingSqm: false, neverDetailed: true }],
    total: 1,
  }));
...
test('MAX_GEO_FETCHES=0 disables the enrichment pass entirely', async () => {
  ...
  const db = fakeDb(() => { queueCalled = true; return { pending: [], total: 0 }; });
  const fetchPage = pageFetcher({
    1: { items: [rawCard(1)], meta: meta(1, 1, 1) },
  });
  const res = await run(db, { maxGeoFetches: 0 }, { fetchSearchPage: fetchPage });
