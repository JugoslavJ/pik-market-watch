...
async function seenVia(key, articleId, over = {}) {
  await db.registerSavedSearch({ searchKey: key, name: 'search ' + key,
                                 url: 'https://olx.ba' + key, category: 'apartments' });
  const card = {
    url: `https://olx.ba/artikal/${articleId}/x`, title: 'ad ' + articleId,
    sqm: 50, rooms: '2', isRent: false,
    price: 100000, priceText: '100.000 KM', ppm2: 2000, ...over,
  };
...
needsDb('a listing shared by several searches stays open until all drop it', async () => {
  await db.registerSavedSearch({ searchKey: KEY_B, name: 'search B',
                                 url: 'https://olx.ba' + KEY_B, category: 'houses' });
  await seenVia(KEY_A, 6005);
  await db.saveCards([{ url: 'https://olx.ba/artikal/6005/x', title: 'dup', sqm: 50, rooms: '2',
                        price: 100000, priceText: '', ppm2: 2000, isRent: false }]);
...
needsDb('result links of deconfigured searches are purged and their listings closed', async () => {
  ...
  const links = await db.pool.query(
    'SELECT count(*)::int AS n FROM search_results WHERE search_key = $1', [KEY_B]);
  assert.equal(links.rows[0].n, 0);
});

needsDb('an empty active-key list is refused (would close everything)', async () => {
  ...
  async function historyIntact() {
    const r = await db.pool.query(
      'SELECT count(*)::int AS n FROM search_results WHERE search_key = $1', [KEY_A]);
    return r.rows[0].n === 1;
  }
});

needsDb('re-sighting a closed ad reopens it and clears closing values', async () => {
  ...
  await db.saveCards([{ url: 'https://olx.ba/artikal/6008/x', title: 'back again', sqm: 50,
                        rooms: '2', price: 88000, priceText: '88.000 KM', ppm2: 1760, isRent: false }]);
...
needsDb('closed listings disappear from v_active_listings immediately', async () => {
