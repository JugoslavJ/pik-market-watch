...
needsDb(
  "enrichListings: fills missing m² and derives ppm² on priced sale ads",
  async () => {
    await seed(4001, { price: 78500 }); // no card m²
    await db.enrichListings([
      { articleId: 4001, latitude: null, longitude: null, sqm: 72 },
    ]);
...
needsDb(
  ...
  async () => {
    ...
    await db.enrichListings([
      { articleId: 4002, latitude: null, longitude: null, sqm: 60 },
    ]);
...
needsDb(
  ...
  async () => {
    ...
    await db.enrichListings([
      { articleId: 4004, latitude: null, longitude: null, sqm: null },
    ]);
    const r = await rowOf(4004);
    assert.equal(Number(r.latitude), 44.78);
    assert.equal(Number(r.longitude), 17.2);
  },
);
...
needsDb(
  "run lifecycle: starts as running, finishRun records the outcome",
  async () => {
    await db.registerSavedSearch(identity);
    const id = await db.startRun(identity.searchKey);
    let r = (
      await db.pool.query("SELECT status FROM scrape_runs WHERE id = $1", [id])
    ).rows[0];
    assert.equal(r.status, "running");
    await db.finishRun(id, { status: "ok", pages: 3, cards: 120 });
    r = (
      await db.pool.query(
        "SELECT status, pages, cards, finished_at FROM scrape_runs WHERE id = $1",
        [id],
      )
    ).rows[0];
