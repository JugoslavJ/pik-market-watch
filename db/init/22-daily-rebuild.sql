-- Forward daily-rebuild optimization.
-- The dirty-cohort/full-rebuild contract remains intact; this is a full
-- literal definition rather than another regexp patch over migration 03.

CREATE OR REPLACE FUNCTION public.analytics_daily_rebuild_window(p_as_of_day date DEFAULT NULL::date)
...
BEGIN
  ...
  SELECT LEAST(
           ((SELECT min(effective_at) FROM public.listing_state_history) AT TIME ZONE 'Europe/Sarajevo')::date,
           ((SELECT min(effective_at) FROM public.listing_price_events) AT TIME ZONE 'Europe/Sarajevo')::date)
    INTO v_first_evidence;
  SELECT min(day) INTO v_first_daily FROM public.listing_daily;
...
-- The installed 19 definition is the source contract; these two changes
-- make article-level first-price and lifecycle activity probes reusable.

CREATE OR REPLACE FUNCTION public.rebuild_listing_daily_legacy(p_from_day date, p_through_day date)
