-- ─────────────────────────────────────────────────────────────────────────────
-- OLX.ba price tracker — PostgreSQL schema (consolidated)
--
-- Single source of truth for the current schema. Squashes the former
-- 01–10 + 12 incremental migrations (2026-08); their evolution lives in git
-- history. Completed by:
--   11-neighborhoods.sql  GENERATED by geo/scripts/gen-sql.js — do not edit
--   zz-database-roles.sh  least-privilege runtime roles
--
-- Idempotent by construction:
--   * tracker-aware volumes (schema_migrations has this filename) skip it;
--   * pre-tracker volumes re-run it harmlessly — tables/indexes use
--     IF NOT EXISTS, the listings self-heal block below re-asserts every
--     column the retired migrations ever added, and functions/views are
--     replaced outright.
--
...
-- Self-heal for volumes predating this consolidated file: re-assert every
-- column the retired incremental migrations added (all were nullable ADDs;
-- none carried data backfills — days-on-market stays a lower bound where
-- published_at was conflated with a renewal stamp, by design).
...
-- DROP + CREATE (the pattern the retired migrations used).
